...
fun EditNoteView(navController: NavController,noteVM : NotesViewModel, idDoc : String ){
    LaunchedEffect(Unit) {
        noteVM.getNoteById(documentId = idDoc)
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(
                value = state.title,
                onValueChange = { noteVM.onValue(it,"title") },
            ...
            OutlinedTextField(
                value = state.note,
                onValueChange = { noteVM.onValue(it,"note")},
